...
export default function SocialShareButtons({ title, url }) {
  ...
  const [canUseNavigatorShare, setCanUseNavigatorShare] = useState(false);
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    setMounted(true);
    setCanUseNavigatorShare(typeof navigator !== 'undefined' && navigator.share);
  }, []);
  
  // Redefine o estado "copied" após 2 segundos
  useEffect(() => {
    if (copied) {
  ...
  // URL de compartilhamento para cada plataforma
  ...
  // Função para abrir links de compartilhamento em uma nova janela
  ...
  return (
    <div className="mt-4">
      <h3 className="text-gray-700 font-medium mb-2">Compartilhar Projeto</h3>
      <div className="flex flex-wrap gap-2">
        {/* Botão para copiar link */}
        <button
          onClick={copyToClipboard}
          className={`${copied ? 'bg-green-600' : 'bg-gray-700 hover:bg-gray-800'} text-white px-3 py-2 rounded-md text-sm flex items-center transition-colors`}
        >
        ...
        <button
          ...
          className="bg-black hover:bg-gray-800 text-white px-3 py-2 rounded-md text-sm flex items-center"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
        ...
        <button
          ...
          className="bg-blue-700 hover:bg-blue-800 text-white px-3 py-2 rounded-md text-sm flex items-center"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
        ...
        {/* API Web Share nativa (apenas dispositivos compatíveis) */}
        {canUseNavigatorShare && (
          <button
            onClick={handleNativeShare}
            className="bg-gray-500 hover:bg-gray-600 text-white px-3 py-2 rounded-md text-sm flex items-center"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
